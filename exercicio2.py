   ...
   if len(sobrenome) > len(padraosobrenome)
      if len(senha) > len(padraosenha)
         if len(email) > len(padraoemail)
          menu = (input("escolha uma das opçoes a seguir:1-Depositar 2-Sacar 3-Conferir Saldo 4-Transferir 5-Encerrar Conta")) 
            if(menu==1):
...
senha = ("admin")
 ...
 menuadm= (input("1-Consultar um cliente 2-Consultar lista de clientes 3-Deletar um cliente 4-Atualizar dados de um cliente específico."))
        if(menuadm==1):
           contacliente=input("Escreva o nome da conta")
           else :
               pass
             if(menuadm==2):
                 listadeclientes [contacliente1,contacliente2,contacliente3]
                 print(listadeclientes)
                 else :
                     pass
                     if(menuadm==3):
                         print(listadeclientes)
                         deletar=(input("insira o numero da conta que deseja deletar"))
                         del listadeclientes [deletar]
                         else :
                             pass
                             if(menuadm==4):
                                 print ("desja atualizar qual dado")
                             else :
                                 pass
